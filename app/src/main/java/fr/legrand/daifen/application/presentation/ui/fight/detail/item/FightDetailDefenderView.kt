...
import kotlinx.android.synthetic.main.view_fight_detail_defender.view.*

class FightDetailDefenderView(
    context: Context?,
    ...
    init {
        inflate(getContext(), R.layout.view_fight_detail_defender, this)
    }

    fun bindItem(item: PlayerViewDataWrapper) {
        view_fight_detail_defender_name.text = item.getName()

        Glide.with(this).load(item.getImageUrl())
            .error(Glide.with(this).load(R.drawable.daifen_login_logo))
            .apply(RequestOptions.circleCropTransform())
            .into(view_fight_detail_defender_image)
    }
}